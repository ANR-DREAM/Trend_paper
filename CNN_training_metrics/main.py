...
def main(cfg: CHLConfig) -> None:
    ...
    if cfg.data.input == 'cmip':
        path = '/home/datawork-lops-oh/biogeo/AI/CNN_CHLORO/CMIP6/'+cfg.data.model_type+'/preprocess/'
        ds_input = xr.open_mfdataset(path + cfg.data.model_type + '_r1i1p1f1_gr_185001-210012.nc').load()
        ds_input = ds_input.rename({'chlos':'chloro'})
    ...
    if cfg.data.input == 'glorys':
        with dask.config.set(**{'array.slicing.split_large_chunks': False}):
            ds_input = xr.open_mfdataset(cfg.path.data_input + 'Glorys_*.nc')
        ds_input = ds_input.sel(latitude = slice(cfg.data.lat_min,cfg.data.lat_max),
                                longitude = slice(cfg.data.lon_min,cfg.data.lon_max))

    if cfg.data.anomaly:
        ds_monthly = ds_input.groupby('time.month')
        monthly_mean = ds_monthly.mean(dim='time')
        ds_input = ds_monthly - monthly_mean
    ...
    ########################
    if cfg.data.output == 'cmip':
        ds_out = ds_input.where(ds_input.mask == 1)
        y_train = ds_out.sel(time = slice(cfg.data.d1_train,cfg.data.d2_train),
                             latitude = slice(cfg.data.lat_min,cfg.data.lat_max),
                             longitude = slice(cfg.data.lon_min,cfg.data.lon_max)).load()
    ...
    if cfg.params.model == 'CNN_M':
        model = CNN(n_channel = n_channel,
                    rateDropout=cfg.params.rateDropout).to(device)
        model_W = CNN_W(n_channel = n_channel,
                    rateDropout=cfg.params.rateDropout).to(device)
    if cfg.params.model == 'UNet':
        model = UNet(n_channels = n_channel, 
                     n_classes = 1,
                     rateDropout=cfg.params.rateDropout, bilinear=False).to(device)
    ...
    ###########
    if cfg.data.input == 'cmip':
        if cfg.data.model_type != cfg.data.model_type_predictors: 
            path = '/home/datawork-lops-oh/biogeo/AI/CNN_CHLORO/CMIP6/'+cfg.data.model_type_predictors+'/preprocess/'
            ds_input = xr.open_mfdataset(path + cfg.data.model_type_predictors + '_r1i1p1f1_gr_185001-210012.nc').load()
            ds_input = ds_input.rename({'chlos':'chloro'})
            ds_input = ds_input.sel(latitude = slice(cfg.data.lat_min,cfg.data.lat_max),
                                longitude = slice(cfg.data.lon_min,cfg.data.lon_max))
            ds_out = ds_input.where(ds_input.mask == 1)
